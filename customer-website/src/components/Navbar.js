...
const Navbar = () => {
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        ...
        <div className="max-w-[1600px] mx-auto px-4 md:px-8">
          <div className="flex items-center justify-between h-20">
            <Link to="/" className="font-heading text-2xl md:text-3xl font-bold tracking-tight">
            ...
            <div className="hidden md:flex items-center gap-8">
              {navLinks.map((link) => (
                <Link key={link.path} to={link.path} className={`font-subheading text-sm tracking-widest hover:text-brand-primary transition-colors ${
            ...
            <div className="flex items-center gap-4">
              <Link to="/cart" className="relative">
                ...
                {getCartCount() > 0 && (
                  <span className="absolute -top-2 -right-2 bg-brand-primary text-black text-xs font-bold w-5 h-5 flex items-center justify-center">
                    {getCartCount()}
                  </span>
                )}
              </Link>
      ...
      {isMobileMenuOpen && (
        <div className="fixed top-20 left-0 right-0 bottom-0 bg-black z-40 md:hidden">
